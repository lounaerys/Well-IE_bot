...
def add_new_log_entry(user_id, **kwargs):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    now = datetime.now().isoformat(timespec='seconds')
    c.execute('''
...
def update_last_user_entry(user_id, **kwargs):
    ...
    if row:
        log_id = row[0]
        vals_log = vals[:-2] + [now, log_id]
        c.execute(f'''
            UPDATE user_log SET {', '.join(fields)}, logged_at = ?
            WHERE id = ?
        ''', vals_log)
    else:
        c.execute('''
...
def get_user_progress_for_periods(user_id, param):
    ...
    no_data_msg = 'нет данных. Добавьте еще одно измерение кнопкой <b>‘➕ Добавить параметры’</b>'

    if not vals:
        return {
            'С последнего измерения': no_data_msg,
            'За неделю': {'value': no_data_msg, 'date': None},
            'За месяц': {'value': no_data_msg, 'date': None},
            'С начала измерений': {'value': no_data_msg, 'date': None}
        }
    ...
    if len(vals) >= 2:
        start_v, start_d = vals[-1]
        res['С начала измерений'] = {
            'value': str(vals[0][0] - start_v),
            'date': start_d.strftime('%d.%m.%Y')
        }
    else:
        start_d = vals[0][1].strftime('%d.%m.%Y')
        res['С начала измерений'] = {
            'value': no_data_msg,
            'date': start_d
        }
